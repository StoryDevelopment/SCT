...
class Trainer(object):
    def __init__(self, config, model):
        ...
        self.grads = self.opt.compute_gradients(zip(*self.loss)[1], var_list=self.var_list)
        tf.summary.scalar('glob_grads',tf.global_norm(list(self.grads)))

        grads, _ = tf.clip_by_global_norm(self.grads, 1)
