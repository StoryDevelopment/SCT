...
flags.DEFINE_string("model_name", "model", "Model name [only4] test_0n_300_-hao")
flags.DEFINE_string("data_dir", "prepro_data/val_cut_300", "Data dir [data/squad]")
flags.DEFINE_string("run_id", "0", "Run ID [0]")
flags.DEFINE_string("out_base_dir", "muti-attention", "out base dir [out]")
flags.DEFINE_string("forward_name", "single", "Forward name [singless]")
...
flags.DEFINE_string("shared_glove_size", "100", "Shared path []")

flags.DEFINE_integer("test_num_can", 2, "candidate test number")
flags.DEFINE_integer("train_num_can", 2, "candidate train number")
# Device placement
...
flags.DEFINE_string("mode", "train", "train | test  [test]")
flags.DEFINE_boolean("load", False, "load saved data? [True]")
flags.DEFINE_integer("load_step", 680, "load step [0]")
...
flags.DEFINE_float("input_keep_prob", 1, "Input keep prob for the dropout of LSTM weights [0.8]")
flags.DEFINE_float("keep_prob", 1, "Keep prob for the dropout of Char-CNN weights [0.8]")
flags.DEFINE_float("output_keep_pro", 1, "Keep prob for the dropout of Char-CNN weights [0.8]")
...
flags.DEFINE_bool("dense_net_first_scale_down_ratio", 0.3, 'dense_net_first_scale_down_ratio')
flags.DEFINE_bool("dense_net_layers", 10, 'dense net layers')
flags.DEFINE_bool("dense_net_growth_rate",  20, 'dense net growth rate')
flags.DEFINE_bool("dense_net_kernel_size", 3, 'dense net kernel size [3]')
flags.DEFINE_bool("dense_net_transition_rate", 0.5, 'dense_net_transition_rate') ##
flags.DEFINE_bool("self_att_logit_func", "tri_linear", 'logit function')
# Optimizations
flags.DEFINE_bool("cluster", False, "Cluster data for faster training [False]")

# Logging and saving options
flags.DEFINE_boolean("progress", True, "Show progress? [True]")
flags.DEFINE_integer("log_period", 20, "Log period [100]")
flags.DEFINE_integer("eval_period", 20, "Eval period [1000]")
flags.DEFINE_integer("save_period", 20, "Save Period [1000]")
...
flags.DEFINE_integer("train_save_min", 240, "train model Save Period min value [240]")
flags.DEFINE_integer("train_save_max", 320, "train model Save Period min value [3400]")

# Thresholds for speed and less memory usage
flags.DEFINE_integer("word_count_th", 3, "word count th [100]")
...
flags.DEFINE_bool("use_pos_emb", True, "use pos tag embedding? [True]")
flags.DEFINE_bool("use_sem_emb", True, "use sentiment embedding? [True]")
flags.DEFINE_bool("use_neg_emb",True, "use negation embedding? [True]")

flags.DEFINE_bool("highway", True, "Use highway? [True]")
flags.DEFINE_bool("hao",True, "use char emb? [True]")
flags.DEFINE_bool("lstm",True, "use char emb? [True]")
flags.DEFINE_bool("self_attention",True, "use char emb? [True]")
flags.DEFINE_bool("interact",True, "use char emb? [True]")
flags.DEFINE_string("plot_encoder",'concate', "use char emb? ['concate' | 'sum' | 'lstmss']")
flags.DEFINE_bool("pos_tag_num", 40, 'the number of pos tag')
...
def main(_):
    ...
    #     config.model_name = "model-highway"
    #     config.out_dir = os.path.join(config.out_base_dir, config.model_name, str(config.run_id).zfill(2))
    #     m(config)
