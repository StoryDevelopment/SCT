...
def zip_save(args):
    ...
    for save_source_path in tqdm(args.paths):
        # path = "out/only4/30/save/only4-18000"
        # target_path = "save_dir/30/save"
        # also output full path name to "save_dir/30/readme.txt
        # need to also extract "out/only4/30/shared.json"
        temp, _ = os.path.split(save_source_path)  # "out/only4/30/save", _
        model_dir, _ = os.path.split(temp)  # "out/only4/30, _
